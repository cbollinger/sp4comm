...
#+ATTR_LATEX: :options [logo=\bcbook, couleur=green!10, barre=snake, arrondi=0.1]{Concert Pitch}
 #+BEGIN_bclogo
...
#+BEGIN_SRC octave :exports both :results file
  ...
  subplot(121),stem(n-N/2,fftshift(abs(X1)))  # Move frequency 0 to the center
  grid on;
  xlabel("Sample");
  ylabel("X_1[k]");
  title("Sepctrum of signal x1");
  subplot(122),stem(n-N/2,fftshift(abs(X2)))
  grid on;
  xlabel("Sample");
  ylabel("X_2[k]");
