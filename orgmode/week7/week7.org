...
****** Sinc interpolation
the ingredients:
- discrete-time signal $x[n] \text{, } n \in \mathbb{Z} \text{ (with DTFT } X(e^{j\omega}))$
...
#+BEGIN_bclogo
The sync function $sinc\left(\frac{t-nT_s}{T_s}\right)$is an orthornormal basis for the $\pi-BL$ space.
#+END_bclogo
...
- *With a Bit-Rate R* of 2 bits is a region split into 4 equally spaced
  ...
  #+begin_src latex
      \begin{align*}
        \sigma_e^2 &= E\left[|Q\left{x[n]\rigth} -x[n]|^2\right] \\
  ...
  #+begin_src latex
    \begin{align*}
        \sigma_e^2 &= \sum_{k=0}^{2^R-1} \int_{A+k\Delta}^{A+k\Delta+k} \frac{(A + k\Delta + \Delta/2 - \tau)^2}{B-A} d\tau \\
                   ...
                   &= \frac{\Delta^2}{12} \text{ with }   \Delta = \frax{B-A}{2^R}
    \end{align*}
  #+end_src
...
#+begin_src latex
  \begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=Signal to Noise Ratio in db]
    \begin{equation*}
        SNR_{db} = 10\cdotlog_{10}2^{2R} \approx 6R \;db
    \end{equation*}
  \end{tcolorbox}
#+end_src
...
| $X(j\Omega) = \frac{\pi}{\Omega_N}X(e^{j\pi\Omega / \Omega_N}) rect\left( \frac{\Omega}{2\OPmega_N}\right)$ |                          | $X(j\Omega) = \frac{\pi}{\Omega_N} I \left(j\pi\frac{\Omega}{\Omega_N} \right) X(e^{j\pi\frac{\Omega}{\Omega_N}})$ |
...
*** TODO Multirate Signal Processing
...
#+BEGIN_SRC octave :exports results :results file
  ...
  subplot(3,1,2)
  plot(omega_p3,XP3, "linewidth", 3, omega_p3,XP3_S,"linewidth", 3), ylabel('X(e^j^\omega)')
  axis([-pi*P,pi*P,0,1]);
  set(gca,'YTick', 0:1)
  set(gca, "fontsize", 20)
...
#+BEGIN_SRC octave :exports results :results file
  ...
  #ans = length(omega_u2)
     ...
     #+BEGIN_SRC octave :exports results :results file
                ...
                X       =  tripuls(omega,pi);
                X       =  tripuls(omega,3*pi/2);
               #X       = [zeros(1,102), X(102:293), zeros(1,102)];
...
**** TODO Sampling Rate Change

#+ATTR_LATEX: :width 7cm
[[./drawing/w7_updownsampler_LP.png]]
*** TODO FIR-based sampling
